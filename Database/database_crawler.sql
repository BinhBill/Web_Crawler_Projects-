...
CREATE TABLE users (
    ...
    role ENUM('admin', 'user', 'reader', 'visitor') DEFAULT 'visitor'
);

CREATE TABLE permissions (
    permission_id INT AUTO_INCREMENT PRIMARY KEY,
    role ENUM('admin', 'user', 'reader', 'visitor') NOT NULL,
...
SELECT * FROM speakers WHERE conference_id = <conference_id>;

---Thông tin về các bài báo được chấp nhận liên quan đến một hội nghị cụ thể
SELECT * FROM papers WHERE conference_id = <conference_id> AND accepted = TRUE;

---Thông tin về các người tham dự đã đăng ký trong một hội nghị cụ thể
SELECT * FROM attendees WHERE conference_id = <conference_id> AND registered = TRUE;
...
WHERE conference.code = <conference_code>;
...
WHERE user_follow_conference.user_id = <user_id>;

---Thông tin về hạn chót của một hội nghị dựa trên conference_code
SELECT * FROM deadline WHERE conference_code = <conference_code>;
